...
/* Causes et corrections par combinaison défaut + filament + extrudeur
   Structure : DIAG_DATA[defaut_id][filament] = { causes, corrections }
   'all' = s'applique à tous les filaments
   extrudeur 'dd' = direct drive, 'bow' = bowden, 'all' = les deux
*/
const DIAG_DATA = {
  ...
  stringing: {
    all: {
      dd: [
        { prob: 'Principal', cause: 'Température de buse trop élevée — le filament coule pendant les déplacements', difficulte: 'Facile', actions: ['Baisser la température de 5°C et retester', 'Activer le "combing" dans le slicer pour éviter les déplacements au-dessus des vides'] },
    ...
    TPU: {
      dd: [
        { prob: 'Principal', cause: 'Le TPU est souple et difficile à rétracter — réduire au maximum', difficulte: 'Moyen', actions: ['Désactiver la rétraction ou la mettre à 0.5mm max', 'Réduire la vitesse d\'impression à 20-30mm/s', 'Activer le combing dans le slicer'] },
      ],
      bow: [
        { prob: 'Principal', cause: 'Le TPU en Bowden est très difficile — le filament souple se comprime dans le tube', difficulte: 'Technique', actions: ['Réduire la vitesse à 15-20mm/s', 'Désactiver la rétraction complètement', 'Envisager de passer en direct drive pour le TPU'] },
      ],
    },
    PETG: {
      all: [
        { prob: 'Principal', cause: 'Le PETG est naturellement collant et prone au stringing', difficulte: 'Moyen', actions: ['Baisser la température de 5-10°C (essayer 230°C)', 'Augmenter la vitesse de déplacement', 'Activer le "wipe on retract" dans le slicer'] },
      ],
    },
  },
  ...
  bulles: {
    ...
    PETG: {
      all: [
        { prob: 'Principal', cause: 'Le PETG est très hygroscopique — les bulles sont souvent dues à l\'humidité', difficulte: 'Facile', actions: ['Sécher le filament 6-12h à 65°C', 'Stocker dans une boîte hermétique avec silicagel'] },
      ],
    },
    Nylon: {
      all: [
        { prob: 'Principal', cause: 'Le Nylon absorbe l\'humidité en 1-2h — séchage indispensable', difficulte: 'Facile', actions: ['Sécher 8-24h à 80°C', 'Imprimer directement depuis le séchoir'] },
      ],
    },
  },
  ...
  surplomb: {
    ...
    ABS: {
      all: [
        { prob: 'Principal', cause: 'L\'ABS ne supporte pas un refroidissement fort — les surplombs sont naturellement plus difficiles', difficulte: 'Moyen', actions: ['Réduire la vitesse sur les surplombs', 'Ajouter des supports dès 40°', 'Ne pas augmenter le ventilateur au-delà de 30% pour éviter le warping'] },
      ],
    },
  },
...
/* Résolution : filament spécifique > 'all', extrudeur spécifique > 'all' */
function getDiagnostic(defautId, filament, extrudeur) {
  const data = DIAG_DATA[defautId];
  if (!data) return [];

  // Cherche filament spécifique, sinon 'all'
  const filData = data[filament] || data['all'];
  if (!filData) return [];

  // Cherche extrudeur spécifique, sinon 'all'
  const causes = filData[extrudeur] || filData['all'];
  return causes || [];
}
